...
*/
	...
	public:
		...
		*/
		...
		/*!
		@brief リソースの解放
		@param[in]	vector<string>keys リソースキーの配列
		@return	なし
		*/
		//--------------------------------------------------------------------------------------
		void UnRegisterResource(const vector<wstring>& Keys) {
			try {
				for (auto key : Keys)
				{
					...
					{
						throw(BaseException(L"指定のキーのリソースがありません",key,L"App::UnRegisterResource()"));
					}
				}
			}
			catch (...) {
